...
const About = () => {
  const skills = [
    ...
    { category: "Databases", items: ["MySQL", "MongoDB", "PostgreSQL", "SQLite"] },
    { category: "Machine Learning", items: ["Scikit-learn", "Keras", "OpenCV", "Natural Language Processing"] }
  ];
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {skills.map((skillGroup, index) => (
                <div key={index} className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg">
                  <h4 className="font-medium text-slate-700 dark:text-slate-300 mb-2">{skillGroup.category}</h4>
                  <div className="flex flex-wrap gap-2">
                    {skillGroup.items.map((skill, skillIndex) => (
                      <span
                        key={skillIndex}
                        className="px-3 py-1 bg-white dark:bg-slate-700 text-sm text-slate-600 dark:text-slate-300 rounded-full shadow-sm"
                      >
                        {skill}
                      </span>
                    ))}
